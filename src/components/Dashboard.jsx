        ...
        {detailsIdx !== null && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: '#000a', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setDetailsIdx(null)}>
            <div style={{ background: '#23263a', color: '#fff', borderRadius: 10, padding: 32, minWidth: 400, maxWidth: 600, boxShadow: '0 4px 32px #0008', position: 'relative' }} onClick={e => e.stopPropagation()}>
              ...
              < JsonView src={rows[detailsIdx]} name={false} collapsed={1} enableClipboard={true} displayDataTypes={false} theme="monokai" style={{ background: 'none', fontSize: 15, borderRadius: 8, padding: 12, maxHeight: 400, overflow: 'auto' }} />
            </div>
          </div>
        )}
      </div>
        ...
        <div style={{
          ...
          {viewMode === 'pretty' && (
            <div style={{ margin: '32px auto', maxWidth: 900, background: '#181a20', borderRadius: 14, boxShadow: '0 2px 12px #2224', padding: 28 }}>
              <JsonView src={normalizedData} name={false} collapsed={2} enableClipboard={true} displayDataTypes={false} theme="monokai" style={{ background: 'none', fontSize: 16, borderRadius: 8, padding: 12 }} />
            </div>
          )}
          {viewMode === 'raw' && (
            <div style={{ margin: '32px auto', maxWidth: 900, background: '#181a20', borderRadius: 14, boxShadow: '0 2px 12px #2224', padding: 28 }}>
              <strong style={{ color: '#fff', fontSize: 18 }}>Raw JSON Response:</strong>
              <JsonView src={data} name={false} collapsed={1} enableClipboard={true} displayDataTypes={false} theme="monokai" style={{ background: 'none', fontSize: 16, borderRadius: 8, padding: 12, marginTop: 10 }} />
            </div>
          )}
        </div>
      )}
